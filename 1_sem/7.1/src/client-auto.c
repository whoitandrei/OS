...
void client_run(int sockfd, struct sockaddr_in server_addr) {
    ...
    char str[BUFFER_SIZE] = "hello from client auto!";
...
int main() {
    ...
    int inet_pton_ret = inet_pton(IPV4, "127.0.0.1", &server_addr.sin_addr); // преобразует ip из строки в бинарник и записывает по указателю
    if (inet_pton_ret <= 0) {
